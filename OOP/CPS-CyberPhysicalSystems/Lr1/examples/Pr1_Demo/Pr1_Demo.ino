...
// Pin assignment (adjust per board)
...
void setup() {
    ...
    manager.add(&rgb);
    manager.initAll();
    Serial.println(F("System init OK"));
}

void loop() {
    unsigned long now = millis();

    // Toggle power strategy with onboard button (if present) or via serial command.
    if(Serial.available()) {
        char c = Serial.read();
    ...
    // Sensor sampling respecting strategy interval
    if(now - lastSensorSample >= currentStrategy->sensorIntervalMs()) {
        ...
        // Map light sensor to color (darker room -> warmer color)
        float light = ldr.lightLevel(); // 0..1
