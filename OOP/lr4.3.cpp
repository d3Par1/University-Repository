...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1) {
      ...
      std::string output = chain.generate(100);
      std::cout << "\nЗгенерований текст без роздільників:" << std::endl;
...
public:
    MarkovChain(int chain_order = 2, bool use_delims = false) : 
        order(chain_order), 
    ...
    void build_from_text(const std::string& text) {
        ...
        if (!use_delimiters && starting_states.empty()) {
            for (const auto& [state, _] : state_transitions) {
                starting_states.push_back(state);
            }
        }
